...
int** levelOrder(struct TreeNode* root, int** columnSizes, int* returnSize) {
    ...
    *returnSize = retSize;
    ...
    struct ListHeadNode* now = headOfListHead->next;

    while( now != NULL ) {

        *( *columnSizes + indexOfRet ) = now->cnt;

        *( ret + indexOfRet ) = calloc( (size_t) now->cnt , sizeof(int) );
        struct ListNode* nowChild = now->child;
        int indexOfThisRank = 0;
        while( nowChild != NULL ) {
            *( *(ret + indexOfRet) + indexOfThisRank ) = nowChild->val;
...
int main() {
    ...
    int** retArr = levelOrder( root , &retArrColSizes , &retArrSize );
